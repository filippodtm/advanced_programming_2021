...
public:
  ...
  // Riserva n nodi con std::vector<node_t>::reserve
  ...
  /* Dà in uscita il valore nel nodo in posizione x (cioè il primo elemento della pila identificata da x)*/
  T& value(stack_type x) noexcept {return node(x).value; }    
  const T& value(stack_type x) const noexcept {return node(x).value; }     

  /*Dà in uscita l'indice del prossimo elemento della pila identificata da x*/
  ...
  /* push aggiunge un nuovo valore alla pila identificata da l*/
